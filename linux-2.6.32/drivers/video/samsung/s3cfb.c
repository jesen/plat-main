...
static int __init s3cfb_probe(struct platform_device *pdev)
{
	...
	printk("s3cfb_probe 9\n");
	return 0;

free_video_memory:
    printk("s3cfb_probe free_video_memory\n");
	s3cfb_unmap_video_memory(&s3c_fb_info[index]);

release_clock:
    printk("s3cfb_probe release_clock\n");
	clk_disable(info->clk);
	clk_put(info->clk);

release_mem:
    printk("s3cfb_probe release_mem\n");
	release_resource(info->mem);
	kfree(info->mem);

release_irq:
    printk("s3cfb_probe release_irq\n");
#if defined(CONFIG_CPU_S3C2443) || defined(CONFIG_CPU_S3C2450) || defined(CONFIG_CPU_S3C2416)
